...
// "연동하기" 버튼을 누르면 ONB-03(HealthConnectStep)으로 화면만 이동한다.
  ...
  return (
    <View style={styles.figmaCanvas}>
      ...
      <Text style={styles.healthSubtitle}>
        SkinCast는 애플 건강 앱의 수면 기록만 읽습니다. 쓰기 권한은 요청하지
        않으며, 언제든 해제할 수 있어요.
      </Text>
      ...
      {/* CTA (node 261:699, x:28 y:705 w:345, gap:8) — "건강 앱 연결하기"는 실제 HealthKit
...
export function OnboardingFlow({ onComplete }: { onComplete: () => void }) {
  ...
  const handleConnectHealthKit = async () => {
    const error = await initHealthKit(); // 여기서 실제 iOS 권한 팝업이 뜸
    if (error) {
    ...
    // uploadSleepSession은 내부에서 모든 에러를 처리하므로 실패해도 onComplete()는 항상 호출된다.
