...
// 앱 시작마다(_layout.tsx) + 온보딩 최초 1회(onboarding-flow.tsx) 두 곳에서 각각 일어나지만
// (useHealthData.ts의 applySleepSessionUploadResult 주석 참고), 팝업(원래 시안 디자인)은 일간
// 리포트 화면을 처음 열었을 때 뜨는 게 맞다 — 두 호출부 모두 이 화면의 부모가 아니라 조상이라
// (사이에 (tabs) 스택이 낌) props로 직접 못 내려주므로, use-account-reset-signal.ts와 동일한
// 이유로 useSyncExternalStore 기반 store를 둔다.
//
// 위 두 호출부 중 하나가 setSleepScoreExpResult로 값을 채우면, daily-report.tsx가
...
export interface SleepScoreExpResult {
  /** "YYYY-MM-DD" — 이 예보/점수가 어느 기상일 것인지. daily-report.tsx가 오늘 리포트일 때만
   * 보여준다(오래된 값이 store에 소비되지 않은 채 남아 다음 날 뜨는 걸 막는 방어). */
  sleepDate: string;
  score: number;
  expGained: number;
}
